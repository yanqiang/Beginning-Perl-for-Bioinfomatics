...
foreach (@DNA) {
	if (/A/) {
		...
		print "!!!!!!!! Error - I don\'t recognize this
base: ";
		print;    # that is equal to: print $_;
		print "\n";
		++$errors;
	}
}
...
#hold the scalars that are being read from the array ($base served that function in the
...
#doing a pattern match on the $_ variable, so it behaves as if you had said $_ =~ /A/,
