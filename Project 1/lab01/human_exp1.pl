...
my $sth = $dbh -> prepare("insert into human_exp (gene_id, HUES64, Endoderm, Ectoderm, Mesoderm, Hepatoblast_like) values(?, ?, ?, ?, ?, ?)");
...
while ( my ( $gene_id, $values ) = each(%exp_genes) ) {
    ...
    #print "$gene, $HUES64, $Endoderm, $Ectoderm, Mesoderm, $Hepatoblast_like";
